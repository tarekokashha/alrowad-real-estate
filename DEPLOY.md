...
3. حطهم في `alrowad/.env` في السطرين الفاضيين:
   ...
   ```
4. **اتأكد إنهم شغالين قبل ما ترفعهم:**
   ```bash
   npm run storage:check
   ```
   بيرفع ملف صغير فعلًا، يقراه، يقارن، ويمسحه. لو فيه غلطة في النسخ هيقولك
   إيه هي بالظبط.
5. بعد ما يعدّي: `npm run vercel:env` — هيرفعهم لـVercel لوحده

> **ليه الفحص ده مهم؟** المفتاح الغلط مبيقولش إنه غلط. الصورة بتترفع،
> والشكل بيظهر عادي، والمشكلة بتبان بعد أول نشر جديد لما أحمد يكون قعد ليلة
> بيرفع فولدر صور. عشر ثواني هنا بتوفر الليلة دي.
